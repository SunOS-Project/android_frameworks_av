...
{
    audio_utils::lock_guard _l(mutex());
    for (const sp<IAfTrack>& track : mTracks) {
...
status_t PlaybackThread::setAppVolume(const String8& packageName,
        const int uid, const float value)
{
    audio_utils::lock_guard _l(mutex());
    for (const sp<IAfTrack>& track : mTracks) {
        if (packageName == track->getPackageName() && uid == track->getUid()) {
...
status_t PlaybackThread::setAppMute(const String8& packageName,
        const int uid, const bool mute)
{
    audio_utils::lock_guard _l(mutex());
    for (const sp<IAfTrack>& track : mTracks) {
        if (packageName == track->getPackageName() && uid == track->getUid()) {
            track->setAppMute(mute);
        }
    }
    return NO_ERROR;
}
