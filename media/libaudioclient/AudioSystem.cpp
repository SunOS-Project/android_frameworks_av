...
status_t AudioSystem::setAppVolume(const String8& packageName, const int uid, const float volume)
{
    const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
    if (af == 0) return PERMISSION_DENIED;
    return af->setAppVolume(packageName, uid, volume);
}

status_t AudioSystem::setAppMute(const String8& packageName, const int uid, const bool mute)
{
    const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
    if (af == 0) return PERMISSION_DENIED;
    return af->setAppMute(packageName, uid, mute);
}
